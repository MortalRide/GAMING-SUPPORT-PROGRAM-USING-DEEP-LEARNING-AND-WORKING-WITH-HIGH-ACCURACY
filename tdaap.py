...
class Bot:
    ...
    def is_on_target(x, y):
        # Hedef noktasının piksel cinsinden genişliği.
        acceptable_radius = 5
        if 960 - acceptable_radius <= x <= 960 + acceptable_radius:
            if 540 - acceptable_radius <= y <= 540 + acceptable_radius:
                return True
        else:
            return False
    ...
    def move_mouse(x, y):
        ...
        Bot.calculate_distance_from_crosshair(x, y)
        dx, dy = Bot.calculate_distance_from_crosshair(x, y)
        win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, dx, dy, 0, 0)

    # Hedefin x ve y eksenlerinde nişangaha olan uzaklığını hesapla.
    def calculate_distance_from_crosshair(x, y):
        dx = (x - 960)
        dy = (y - 540)
        return dx, dy
    ...
    def deviate_to_moving_target(deviation, x, y):
        scale = Bot.sensitivity_config["mouse_sens"]
        scaled_deviation = int(deviation * 2.5 / scale * 2.5)
        Bot.move_mouse(x + scaled_deviation, y)
    ...
    def start(self):
        ...
        Bot.toggle_autoaim()
        # Program 1920x1080 çözünürlüğe sahip bir sistem üzerinde oluşturulup denendiği için farklı çözünürlüğe sahip sistemler üzerinde doğru çalışmayabilir.
        screen_width_halved, screen_height_halved = ctypes.windll.user32.GetSystemMetrics(0)/2, ctypes.windll.user32.GetSystemMetrics(1)/2
        detection_box = {'left': int(screen_width_halved - self.detection_box_size // 2),  # Algılama yapılacak kutunun sol üst köşesinin x koordinatını hesaplar
                          'top': int(screen_height_halved - self.detection_box_size // 2),  # Algılama yapılacak kutunun sol üst köşesinin y koordinatını hesaplar
        ...
        while True:
            ...
            if len(results.xyxy[0]) != 0: # Hedef algılandı.
                for *box, conf, cls in results.xyxy[0]: # Algılanan tüm hedefler için tekrar et.
                    x1y1 = [int(x.item()) for x in box[:2]] # Algılanan hedeflerin çerçevelerinin sol alt köşe koordinatları.
                    x2y2 = [int(x.item()) for x in box[2:]] # Algılanan hedeflerin çerçevelerinin sağ üst köşe koordinatları.
                ...
                if closest_target: # Geçerli bir tespit varsa.
                    ...
                    if Bot.is_on_target(head_exact_X, head_exact_Y): # Nişangah hedefin kafa bölgesi üzerine geldiğinde "ON TARGET" mesajını görüntü ekranına bas.
                        Bot.l_click() # Ateş et
                    ...
                    if Bot.is_autoaim_on(): # Otomatik nişan alma açık ise en yakın hedefin kafa koordinatlarını kullanarak fare hareket metodlarını çağır.
                        if Bot.is_moving_targets_enabled(): # Hareketli hedef takibi açıksa hareket yönünü ve derecesini hesaplayarak sapma fonksiyonlarını çağır.
                            if -20 < head_exact_X - last_x < 20 and not (-1 < head_exact_X - last_x < 1):
                                ...
                                m_counter += 1
                            if m_counter >= 3:
                                scaled_deviation = Bot.deviate_to_moving_target(deviation, head_exact_X, head_exact_Y)
                                ...
                                Bot.move_mouse(head_exact_X, head_exact_Y)
                                last_x = head_exact_X
                        else:
                            Bot.move_mouse(head_exact_X, head_exact_Y)
